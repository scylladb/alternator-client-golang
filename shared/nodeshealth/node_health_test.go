...
func TestNodeEventScore(t *testing.T) {
	...
	cases := []struct {
		...
		{"success", nil, 0},
		{"context deadline", context.DeadlineExceeded, DefaultNodeEventScoreWeights.Timeout},
		...
		{"net temporary", &net.DNSError{IsTemporary: true}, DefaultNodeEventScoreWeights.Temporary},
		{"connection refused", &net.OpError{Err: syscall.ECONNREFUSED}, DefaultNodeEventScoreWeights.ConnectionRefused},
		{"tls handshake", &tls.RecordHeaderError{}, DefaultNodeEventScoreWeights.TLSHandshake},
...
func TestNodeHealthStoreScoreAdjustments(t *testing.T) {
	...
	if status == nil {
		t.Fatalf("expected initial node status")
	}
...
func TestNodeHealthStoreGetActiveNodesTriggersAsyncRelease(t *testing.T) {
	t.Parallel()

	node := url.URL{Scheme: "http", Host: "node4:8080"}
	triggered := make(chan struct{}, 1)

	cfg := DefaultNodeHealthStoreConfig()
	initial := []url.URL{node}
	cfg.Scoring.QuarantineScoreCutOff = 1
	releaseFunc := func(_ url.URL, _ NodeHealthStatus) bool {
		select {
	...
	cfg.ActiveCheckReleaseFrequency = 2
	store, err := NewNodeHealthStore(cfg, releaseFunc, initial)
	if err != nil {
		t.Fatal(err)
	}
	...
	select {
	case <-triggered:
		t.Fatalf("release triggered before threshold reached")
	default:
	}
	...
	case <-time.After(time.Second):
		t.Fatalf("expected release callback after threshold reached")
	}
}
